...
static vector<vector<CacheLine>> cache;
static unsigned int g_numSets = 0;
void initCache(unsigned int ways)
{
    g_ways = ways;
    g_writebacks = 0;

    // reset replacement RNG
    m_w_rep = 0x12345678;
    m_z_rep = 0x9ABCDEF0;

    // Number of sets
    g_numSets = CACHE_SIZE / (LINE_SIZE * ways);

    // Allocate cache
    cache.clear();
    cache.resize(g_numSets);

    for (unsigned int s = 0; s < g_numSets; s++)
    {
        cache[s].resize(ways);
...
{
    // Address breakdown
    unsigned int blockAddr = addr / LINE_SIZE;

    unsigned int setIndex = blockAddr % g_numSets;

    unsigned int tag = blockAddr / g_numSets;

    // -----------------------------
    // Search for hit
    // -----------------------------
    for (unsigned int i = 0; i < g_ways; i++)
    {
        CacheLine &line = cache[setIndex][i];
    ...
    unsigned int victim = randReplace() % g_ways;

    CacheLine &line = cache[setIndex][victim];
